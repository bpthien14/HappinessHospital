...
class Patient(models.Model):
    """
    Patient model for Hospital Management System (Microservice version)
    ...
    has_insurance = models.BooleanField(default=False, help_text="Có BHYT không")
    ...
    insurance_valid_from = models.DateField(
        blank=True, 
        null=True,
        help_text="BHYT có hiệu lực từ ngày"
    )
    insurance_valid_to = models.DateField(
        blank=True, 
        null=True,
        help_text="BHYT có hiệu lực đến ngày"
    )
    ...
    # System Information (using UUID instead of ForeignKey)
    created_by_id = models.UUIDField(
        null=True, 
        blank=True,
        help_text="ID của người tạo hồ sơ (from Auth Service)"
    )
    updated_by_id = models.UUIDField(
        null=True, 
        blank=True,
        help_text="ID của người cập nhật cuối (from Auth Service)"
    )
...
class MedicalRecord(models.Model):
    """
    Medical Record model for Hospital Management System (Microservice version)
    Note: No direct User model dependencies - uses UUID references instead
    """
    VISIT_TYPES = [
    ...
    patient_id = models.UUIDField(help_text="ID của bệnh nhân")
    doctor_id = models.UUIDField(
        null=True, 
        blank=True,
        help_text="ID của bác sĩ khám (from Auth Service)"
    )
    ...
    heart_rate = models.IntegerField(null=True, blank=True, help_text="Nhịp tim")
    ...
    created_by_id = models.UUIDField(
        null=True, 
        blank=True,
        help_text="ID của người tạo (from Auth Service)"
    )
    updated_by_id = models.UUIDField(
        null=True, 
        blank=True,
        help_text="ID của người cập nhật (from Auth Service)"
    )
    ...
    class Meta:
        ...
        ordering = ['-visit_date']
        indexes = [
            models.Index(fields=['patient_id', 'visit_date']),
            models.Index(fields=['doctor_id', 'visit_date']),
            models.Index(fields=['visit_date']),
            models.Index(fields=['status']),
        ]
    
    def __str__(self):
        return f"{self.medical_record_number} - {self.visit_date.strftime('%d/%m/%Y')}"
    ...
    def generate_record_number(self):
        """Tạo mã hồ sơ bệnh án tự động: HS + YYYYMM + 4 số"""
        from django.utils import timezone
        
        today = timezone.now()
        year_month = today.strftime('%Y%m')
        prefix = f"HS{year_month}"
        
        # Get the last medical record for this month
...
class PatientDocument(models.Model):
    """
    Patient Document model for Hospital Management System (Microservice version)
    Note: No direct User model dependencies - uses UUID references instead
    """
    DOCUMENT_TYPES = [
        ('ID_CARD', 'CCCD/CMND'),
    ...
    file_url = models.URLField(max_length=500, help_text="URL của file tài liệu")
    ...
    uploaded_by_id = models.UUIDField(
        null=True, 
        blank=True,
        help_text="ID của người tải lên (from Auth Service)"
    )
    ...
    def __str__(self):
        return f"{self.title} - {self.uploaded_at.strftime('%d/%m/%Y')}"
    
    def save(self, *args, **kwargs):
        # Note: file_size calculation removed since we use URL instead of FileField
        super().save(*args, **kwargs)
