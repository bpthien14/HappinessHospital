...
async function createPatientsChart() {
    try {
        showChartLoading('patients-chart-loading');
        
        const chartData = await getPatientsMonthlyData();
        
        const ctx = document.getElementById('patientsChart').getContext('2d');
        
        if (charts.patientsChart) {
            charts.patientsChart.destroy();
        }
        
        charts.patientsChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: chartData.labels,
                datasets: [{
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                return `Bệnh nhân mới: ${context.parsed.y}`;
                            }
                        }
                    }
                },
        ...
        hideChartLoading('patients-chart-loading');
        document.getElementById('patientsChart').style.display = 'block';
        
    } catch (error) {
        console.error('❌ Error creating patients chart:', error);
        showChartError('patients-chart-loading', 'Lỗi khi tải biểu đồ bệnh nhân');
    }
}
...
async function refreshCharts() {
    ...
    try {
        ...
        ['appointmentsChart', 'appointmentStatusChart', 'patientsChart', 'prescriptionsChart'].forEach(chartId => {
...
async function getPatientsMonthlyData() {
    try {
        ...
        console.log('📈 Patients monthly data:', { labels: months, data: data });
